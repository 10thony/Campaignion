...
exports.demonstrateChatService = demonstrateChatService;
...
async function demonstrateChatService() {
    ...
    for (const [userId] of participants) {
        const subscriptionId = eventBroadcaster.subscribe('demo-interaction', ['CHAT_MESSAGE'], (event) => {
    ...
    try {
        ...
        const partyMessage1 = await chatService.sendMessage('demo-interaction', 'player1', 'Hello everyone! Ready for adventure?', 'party', undefined, 'char1');
        chatService.addMessageToHistory(gameState, partyMessage1);
        const partyMessage2 = await chatService.sendMessage('demo-interaction', 'player2', 'Let\'s do this!', 'party', undefined, 'char2');
        chatService.addMessageToHistory(gameState, partyMessage2);
        console.log('\n2. Sending DM message...');
        const dmMessage = await chatService.sendMessage('demo-interaction', 'dm', 'You see a mysterious door ahead...', 'dm', undefined, 'dm-npc');
        chatService.addMessageToHistory(gameState, dmMessage);
        console.log('\n3. Sending private message...');
        const privateMessage = await chatService.sendMessage('demo-interaction', 'player1', 'I think we should be careful here', 'private', ['player2'], 'char1');
        chatService.addMessageToHistory(gameState, privateMessage);
        console.log('\n4. Sending system message...');
        const systemMessage = await chatService.sendSystemMessage('demo-interaction', 'Combat has begun! Roll for initiative.');
        chatService.addMessageToHistory(gameState, systemMessage);
        console.log('\n5. Testing profanity filter...');
        const filteredMessage = await chatService.sendMessage('demo-interaction', 'player1', 'This damn puzzle is stupid!', 'party', undefined, 'char1');
        ...
        try {
            for (let i = 0; i < 12; i++) {
                await chatService.sendMessage('demo-interaction', 'player1', `Rapid message ${i}`, 'party');
            }
        }
        catch (error) {
            console.log(`Rate limiting triggered: ${error.message}`);
        }
...
//# sourceMappingURL=ChatServiceExample.js.map