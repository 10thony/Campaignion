...
export const MapInstanceManager: React.FC<MapInstanceManagerProps> = ({
  ...
  const handleCreateInstance = async () => {
    if (!isSignedIn || !user) {
      setError('Please sign in to create a map instance');
      return;
    }
  ...
  const handleUndoMove = async () => {
    ...
    try {
      await undoLastMove({ instanceId: instance._id, clerkId: user?.id });
      setError(null);
    } catch (err) {
  ...
  const handleResetInstance = async () => {
    ...
    try {
      await resetMapInstance({ instanceId: instance._id, clerkId: user?.id });
      setError(null);
    } catch (err) {
