...
export function validateRouterInput<K extends keyof typeof RouterInputSchemas>(
  procedure: K,
  input: unknown
): z.infer<typeof RouterInputSchemas[K]> {
  const schema = RouterInputSchemas[procedure];
  return schema.parse(input);
}

// Utility function to validate router outputs
export function validateRouterOutput<K extends keyof typeof RouterOutputSchemas>(
  procedure: K,
  output: unknown
): z.infer<typeof RouterOutputSchemas[K]> {
